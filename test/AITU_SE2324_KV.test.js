...
describe("AITU_SE2324_KV Token", function () {
  let Token, token, owner, addr1, addr2;

  beforeEach(async function () {
    Token = await ethers.getContractFactory("AITU_SE2324_KV");
    [owner, addr1, addr2] = await ethers.getSigners();
    token = await Token.deploy(2000);
  });

  it("Should have correct name and symbol", async function () {
    expect(await token.name()).to.equal("AITU_SE2324_KV");
  ...
  it("Should transfer tokens correctly", async function () {
    ...
    expect(await token.balanceOf(owner.address)).to.equal(2000n * 10n ** 18n - amount);
  });
  ...
  I'm Commenting These two as they are not actually what was asked from task 3
  I did Allowance and transferFrom because I thought 
  I had to overwrite these functions for my coin to work.
  ...
  it("Should return latest transaction timestamp", async function () {
    ...
    expect(timeString).to.include("seconds ago"); // If it returns something, it returns AT LEAST something
  });
});
